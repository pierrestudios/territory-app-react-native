...
export default class UserDetails extends React.Component {
  static navigationOptions = {
	...
	editUser = (data) => {
		NavigationService.navigate('UserEdit', {data, updateUser: this.updateUser});
	}
	...
	updateUser = (data, unattachedPublishers = null) => {
		this.setState({data, unattachedPublishers: unattachedPublishers || this.state.unattachedPublishers}, () => {
			if (typeof this.props.navigation.getParam('updateUsers') === 'function') {
        this.props.navigation.getParam('updateUsers')(data, unattachedPublishers);
			}
		})
	}
    ...
    if (!!territory) {
      ...
      if (typeof this.props.navigation.getParam('updateUser') === 'function') {
        this.props.navigation.getParam('updateUser')(data, unattachedPublishers);
      }
    })
  }
	render() {
      ...
      <View style={[styles.section, styles.content]}>
				<View style={[styles['territory-heading'], {height: 125}]}>
					<Text style={[styles['heading-name'], styles['heading-publisher-name']]}>
						{state.data.publisher ? state.data.publisher.firstName + ' ' + state.data.publisher.lastName : Language.translate('No Publisher')} 
					</Text>
					<Text style={[styles['heading-name'], styles['heading-user-email'], {top: 55, fontWeight: 'bold'}]}>
						{UTILS.userTypeLabel(state.data.userType)}
					</Text>
					<Text style={[styles['heading-name'], styles['heading-user-email'], {top: 75}]}>
						{state.data.email}
					</Text>
				</View>  
