...
export default class Users extends React.Component {
  static navigationOptions = {
	...
	getListings(data = []) {
		return (
			<FlatList
				...
				renderItem={({item}) => (
					<Swipeout key={item.userId} right={[
						{ text: Language.translate('Details'), type: 'primary', onPress: () => this.viewDetails(item) },
            { text: Language.translate('Edit'), backgroundColor: colors.green, onPress: () => this.editUser(item) }, 
            { text: Language.translate('Delete'), type: 'delete', onPress: () => this.notifyDelete(item, state.user) } // backgroundColor: colors.red  
					...
					<View style={[styles['listings-item']]}>
						<TouchableOpacity 
							style={[styles['listings-name'], styles['user-listings-name'], {width: '95%'}]} 
							onPress={() => this.viewDetails(item)}
							>
							<View style={[styles['listings-name'], {marginTop: 5, left: 5, width: '50%'}]}>
								{item.publisher ? 
								<Text numberOfLines={1} style={[styles['listings-name-text'], {fontWeight: 'bold', color: colors.black}]}>
									{(item.publisher.firstName + ' ' + item.publisher.lastName)}
								</Text>
								: 
								<Text numberOfLines={1} style={[styles['listings-name-text'], {color: colors["grey-lite"]}]}>
							...
							<View style={[styles['listings-email'], {marginTop: -3} ]}>
								...
								<Text numberOfLines={1} style={styles['listings-email-text']}>{item.email}</Text>
							</View>	
						</TouchableOpacity>
	...
	updateUsers = (updatedUser, unattachedPublishers = null) => {
		this.setState({
      unattachedPublishers: unattachedPublishers ? unattachedPublishers.sort(UTILS.sortPublisher) : this.state.unattachedPublishers,
