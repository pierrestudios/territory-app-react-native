...
export default class PublisherEdit extends React.Component {
  static navigationOptions = ({ navigation }) => {
    return {
      ...
      headerRight: (
				<ButtonHeader
					onPress={() => {navigation.setParams({savePublisher: true})}}
					title="Save"
					color="#fff"
        />
			),
  ...
  state = {
    ...
    if (!this.state.data.firstName || !this.state.data.firstName)
			return this.setState({errors: {
				...this.state.errors,
				firstName: !this.state.data.firstName ? Language.translate('Enter First Name') : '',
				lastName: !this.state.data.lastName ? Language.translate('Enter Last Name') : '',
    ...
    const url = !!this.state.data.publisherId ? `publishers/${this.state.data.publisherId}/save` : `publishers/add`;
    ...
    .then(res => {
      ...
      if (!!this.state.data.publisherId && typeof this.props.navigation.getParam('updatePublisher') === 'function') {
      ...
      else if (typeof this.props.navigation.getParam('addPublisher') === 'function') {
        const newPublisher = {...this.state.data}
