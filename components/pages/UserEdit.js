...
export default class UserEdit extends React.Component {
  static navigationOptions = ({ navigation }) => {
    return {
      ...
      headerRight: (
				<ButtonHeader
					onPress={() => {navigation.setParams({saveUser: true})}}
					title="Save"
					color="#fff"
        />
			),
      ...
      <View style={[style.container]}>
        <KeyboardAwareScrollView 
          ...
          <SelectBox 
            ...
            value={UTILS.userTypes.find(t => t.value === state.data.userType) || {}} 
  ...
  saveOptionData = (data) => {
	...
	saveUser = () => {
    if (!this.state.data.userType || !this.state.data.email || !UTILS.validEmail(this.state.data.email))
